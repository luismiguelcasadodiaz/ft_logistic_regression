...
def main(dataset_path):
    ...
    features = df.columns
    courses = [f for f in features[4:] if not f.endswith("_missing")]
    print(courses)
    ...
    for i in range(n_courses):
        for j in range(n_courses):
            ...
            ax = axes[idx]
            if j <= i:
                ax.scatter(df[courses[j]], df[courses[i]],
                           marker='.', s=1, c=colors)
            if idx % n_courses == 0:
    ...
    plt.subplots_adjust(hspace=0.00,
                        ...
                        left=0.019,
                        right=1)
    plt.savefig('media/Hogwarts_Courses_Scatter_plots_color', dpi=300)
    plt.show()
